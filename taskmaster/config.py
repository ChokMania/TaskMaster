    ...
    def validate_config(self):
        ...
        for program_name, program in self.data["programs"].items():
            ...
            if not isinstance(program["starttime"], int) or program["starttime"] <= 0:
                raise ValueError(
                    "The 'starttime' field must be an integer greater than 0"
                )

            if not isinstance(program["stoptime"], int) or program["stoptime"] <= 0:
                raise ValueError(
                    "The 'stoptime' field must be an integer greater than 0"
                )
